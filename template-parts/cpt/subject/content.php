<?php
/**
 * The template for displaying the Subject CPT content.
 *
 * @package ArmandPhilippot-com
 * @since 0.0.2
 */

$apcom_current_subject_id    = get_the_ID();
...
$apcom_current_subject_query = new WP_Query(
	array(
		...
		'meta_query'        => array( // phpcs:ignore WordPress.DB.SlowDBQuery
			array(
				'key'     => 'posts_in_subject',
				'value'   => $apcom_current_subject_id,
				'compare' => 'LIKE',
			),
		),
	)
...
<div id="page__content" class="page__body page__body--cpt" itemprop="articleBody">
	...
	<h2>
		<?php
		printf(
			// translators: %s The subject name.
			esc_html__( 'All articles in %s', 'APCom' ),
			esc_html( $apcom_current_subject_name )
		);
		?>
	</h2>
	<?php
	if ( $apcom_current_subject_query->have_posts() ) {
		while ( $apcom_current_subject_query->have_posts() ) {
			$apcom_current_subject_query->the_post();
			$apcom_posted_in_thematic = get_post_meta( get_the_ID(), 'posts_in_thematic' );
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemscope itemtype="http://schema.org/BlogPosting">
				...
				<footer class="article__footer">
					...
					<dl class="article__meta meta">
						...
						<?php
						if ( $apcom_posted_in_thematic ) {
							$apcom_thematics_number = count( $apcom_posted_in_thematic[0] );
							?>
							<div class="meta__item meta__item--has-icon meta__themes">
								...
								<?php
								foreach ( $apcom_posted_in_thematic[0] as $apcom_thematic_id ) {
									$apcom_article_thematic_link = get_permalink( $apcom_thematic_id );
									$apcom_article_thematic_name = get_the_title( $apcom_thematic_id );
									?>
						...
						if ( $apcom_posted_in_subject ) {
							$apcom_this_subject = array( $apcom_current_subject_id );
							$apcom_subject_diff = array_diff( $apcom_posted_in_subject[0], $apcom_this_subject );
							if ( ! empty( $apcom_subject_diff ) ) {
								$apcom_subjects_number = count( $apcom_subject_diff );
								?>
								<div class="meta__item meta__item--has-icon meta__themes">
									<dt class="meta__term">
										<?php
									...
									<?php
									foreach ( $apcom_subject_diff as $apcom_subject_id ) {
